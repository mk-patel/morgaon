...
$query="select * from khairjhitikalan inner join users on khairjhitikalan.user_id=users.id where khairjhitikalan.id1='$eid'";
$result=mysqli_query($conn,$query);
...
<div class="all">
    ...
    <h4 style="color:rgb(153, 0, 51);margin-left:67px; font-size:14px;line-height:20px;"><?php echo htmlspecialchars($row["name"], ENT_QUOTES, 'UTF-8');?></h4></a>
		  ...
		  <?php echo htmlspecialchars($row["date1"], ENT_QUOTES, 'UTF-8'); ?> . <?php echo htmlspecialchars($row["date2"], ENT_QUOTES, 'UTF-8');?></p>
...
<div class="l-side">  
	    ...
	    $text = htmlspecialchars($row["data"], ENT_QUOTES, 'UTF-8');
		...
		<p>
						...
						<?php
							$query1=mysqli_query($conn,"select * from `like_unlike` where postid='".$row['id1']."' and userid='$mid'");
					...
					<span id="show_like<?php echo $row['id1']; ?>" class="likenum">
						<?php
							$query3=mysqli_query($conn,"select * from `like_unlike` where postid='".$row['id1']."' && type='$type1'");
							echo mysqli_num_rows($query3);
						?>
					</span> &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp; <button class="comm"><img style="width:17px;height:16px;" src="images/comm.jpg"> <?php echo $count; ?></button> &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp; <button class="comm"> Share </button>
...
<br>
  ...
  <?php
    ...
    $comm = mysqli_query($conn,"select name,comment,user_id,date2,date1 from comments where post_id='$post_id' and type='ownvillage'order by id desc");
