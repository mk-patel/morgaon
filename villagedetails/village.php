...
// taking village name into eid variable from url.
...
<body>
	...
	<div class="bg-bd">
		<div class="below-header">
			<img src="../sys_images/uservillage.png"/>&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $eid ?>
			&nbsp;&nbsp;
			...
			<div class="container">
				...
				$eid_post_query = "SELECT 
											...
											WHERE p.post_village='$eid' 
											ORDER BY p.post_id desc 
											LIMIT 0,9";
				$eid_post_result=mysqli_query($conn, $eid_post_query);
				$eid_post_count = mysqli_num_rows($eid_post_result);
				
				if($eid_post_count==0){
					echo "<center><br/>No News...<br/><br/></center>";
				}
				while($eid_post = mysqli_fetch_assoc($eid_post_result)){
					$post_id=$eid_post["post_id"];
					$post_date = $eid_post['post_date'];
					$post_time = $eid_post['post_time'];
					...
					$eid_comment_query = "SELECT id FROM comments WHERE post_id='$post_id'";
					$eid_comment_result = mysqli_query($conn, $eid_comment_query);
					$eid_comment_count = mysqli_num_rows($eid_comment_result);
				?>
					<div class="col-md-4">
						<div class="thumbnail">
							<div class="caption">
								<div class="left-side">
									<a href="../userprofile/showprofile.php?eid=<?php echo $eid_post["user_id"];?>">
										<div class="showimg">
											<img class="img-responsive img-thumbnail" src="../<?php echo $eid_post["user_image"]; ?>" />
										</div>
										<h4><?php echo htmlspecialchars($eid_post["user_name"], ENT_QUOTES, 'UTF-8');?></h4>
									</a>
									<p>
							...
							<a href="../post/expand-post-data.php?eid=<?php echo $eid_post["post_id"];?>">
							<div class="l-side">
								<div class="news-data">
									<p class="p1">
										<?php echo htmlspecialchars($eid_post["post_title"], ENT_QUOTES, 'UTF-8');?>
									</p> 
									<p class="p2">
										<?php
										$post_data = htmlspecialchars($eid_post["post_data"], ENT_QUOTES, 'UTF-8');
							...
							<img src="../<?php echo $eid_post["post_image"];?>" alt="image" style="width:100%">
							</a>
							<hr/>
							<p class="action-like">
								<?php
								...
								$like_unlike_query=mysqli_query($conn,"select id from `like_unlike` where postid='".$eid_post['post_id']."' and userid='$mid'");
								// If liked then button "Liked" will be in active state.
								if (mysqli_num_rows($like_unlike_query)>0){
									?>
									<button  value="<?php echo $eid_post['post_id']; ?>" class="unlike">Liked</button>
									<?php
								}
								// If unliked then button "Like" will be in active state.
								else{
									?>
									<button value="<?php echo $eid_post['post_id']; ?>" class="like">Like</button>
									<?php
								}
								?>
								<span id="show_like<?php echo $eid_post['post_id']; ?>" class="likenum">
									<?php
									
									// Showing the like count from "like_unlike" table.
									$like_unlike_count_query=mysqli_query($conn,"select id from `like_unlike` where postid='".$eid_post['post_id']."'");
									echo mysqli_num_rows($like_unlike_count_query);
									?>
								</span> &nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp; 
								<a href="../post/expand-post-data.php?eid=<?php echo $eid_post["post_id"];?>">
									<button class="comm"><img src="../images/comm.jpg"/> <?php echo $eid_comment_count; ?></button>
								</a>&nbsp;&nbsp;&nbsp;|&nbsp;&nbsp;&nbsp; 
								<!-- This button will work only in mobile phones as it will share the text data in whatsapp.-->
								<button class="comm">
									<a href="whatsapp://send?text=<?php echo $eid_post['post_data']; ?>">Share</a>
								</button>
							</p>
						</div>
					</div>
	...
	<nav class="navbar navbar-expand-sm bg-light fixed-bottom">
		<div class="d-flex justify-content-between footer-bottom-icon">
			<div class="footer-button">    
				<a href="../shop/shop.php?eid=<?php echo $eid; ?>"><img src="../images/shop.png" alt="shop"><br>दुकान</a> 
			</div>
			<div class="footer-button">
				<a href="../tution/tution.php?eid=<?php echo $eid; ?>"><img src="../images/tution.png" alt="tution"><br>शिक्षण सुविधा</a>
			</div>
			<div class="footer-button">
				<a href="../bus/bus.php?eid=<?php echo $eid; ?>"><img src="../images/bus.png" alt="Bus"><br>बस सुविधा</a> 
			</div>
			<div class="footer-button">
				<a href="../villagedetails/village-details.php?eid=<?php echo $eid; ?>" ><img src="../images/village.png" alt="village"><br>जानकारी</a>
			</div>
		</div>
	</nav>
